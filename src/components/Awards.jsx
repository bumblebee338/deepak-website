import { motion } from 'framer-motion'
import { useTheme } from '../context/ThemeContext'
import { profile } from '../data/profile'
import { Award, Star } from 'lucide-react'

export default function Awards() {
  const { isDark } = useTheme()

  const containerVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.1,
      },
    },
  }

  const itemVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: {
      opacity: 1,
      y: 0,
      transition: { duration: 0.5 },
    },
  }

  return (
    <section id="awards" className={`py-20 px-4 md:px-8 transition-colors duration-300 ${
      isDark ? 'bg-slate-900/50' : 'bg-white'
    }`}>
      <div className="max-w-6xl mx-auto">
        {/* Section heading */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          viewport={{ once: true }}
          className="text-center mb-16"
        >
          <div className="flex items-center justify-center gap-4 mb-4">
            <div className={`h-1 w-8 rounded-full ${isDark ? 'bg-gold' : 'bg-navy'}`} />
            <span className={`text-sm font-semibold uppercase tracking-wider ${
              isDark ? 'text-gold' : 'text-navy'
            }`}>Recognition</span>
            <div className={`h-1 w-8 rounded-full ${isDark ? 'bg-gold' : 'bg-navy'}`} />
          </div>
          <h2 className={`text-4xl md:text-5xl font-bold ${
            isDark ? 'text-white' : 'text-slate-900'
          }`}>
            Awards & Recognition
          </h2>
        </motion.div>

        {/* Awards grid */}
        <motion.div
          className="grid md:grid-cols-2 gap-8"
          variants={containerVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true }}
        >
          {profile.awards.map((award, index) => (
            <motion.div
              key={index}
              variants={itemVariants}
              className={`group relative overflow-hidden rounded-xl border-2 transition-all duration-300 ${
                isDark
                  ? 'bg-slate-800/50 border-slate-700 hover:border-gold'
                  : 'bg-slate-50 border-slate-200 hover:border-navy'
              }`}
              whileHover={{ y: -4 }}
            >
              {/* Icon area */}
              <div className={`h-32 flex items-center justify-center text-5xl transition-all duration-300 ${
                isDark
                  ? 'bg-gradient-to-br from-gold/10 to-transparent group-hover:from-gold/20'
                  : 'bg-gradient-to-br from-navy/10 to-transparent group-hover:from-navy/20'
              }`}>
                <Star className={isDark ? 'text-gold' : 'text-navy'} size={48} />
              </div>

              {/* Content */}
              <div className="p-6">
                {/* Title with verification badge */}
                <div className="flex items-start justify-between gap-3 mb-2">
                  <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                    {award.title}
                  </h3>
                  <div className="px-2 py-1 bg-gold/10 text-gold rounded-full flex items-center gap-1 flex-shrink-0">
                    <Award size={14} />
                  </div>
                </div>

                {/* Issuer */}
                <div className={`text-sm font-semibold mb-2 ${
                  isDark ? 'text-gold' : 'text-navy'
                }`}>
                  {award.issuer}
                </div>

                {/* Description */}
                <p className={`text-sm leading-relaxed mb-4 ${
                  isDark ? 'text-slate-400' : 'text-slate-600'
                }`}>
                  {award.description}
                </p>

                {/* Year */}
                <div className={`text-xs font-semibold uppercase tracking-wide ${
                  isDark ? 'text-slate-500' : 'text-slate-500'
                }`}>
                  {award.year}
                </div>
              </div>

              {/* Hover overlay */}
              <div className={`absolute inset-0 opacity-0 group-hover:opacity-5 transition-opacity duration-300 pointer-events-none ${
                isDark ? 'bg-gradient-to-br from-gold' : 'bg-gradient-to-br from-navy'
              }`} />
            </motion.div>
          ))}
        </motion.div>
      </div>
    </section>
  )
}
